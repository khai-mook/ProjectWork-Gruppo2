...
//============================METODI GET========================================
	...
	@Override
	public List<ItemType> getItemTypesById(int categoryId){
		List<ItemType> ris = new ArrayList<>();
		List<Map<String, String>> maps = getAll("select * from itemtypes where categoryid=?", categoryId);
		for(Map<String, String> map : maps) {
			ItemType type = IMappablePro.fromMap(ItemType.class, map);
			Category category = IMappablePro.fromMap(
					Category.class, 
					getOne("select * from categories where id=?", categoryId)
					);
			type.setCategory(category);
			List<Map<String, String>> itemMaps = getAll("select * from items where typeid=?", map.get("id"));
			List<Item> items = new ArrayList<>();
...
//============================METODI CRUD=======================================
	...
	public boolean updateDiscount(int id, int discount) {
		return isExecute("update itemtypes set discount=? where id=?", discount, id);
	}
	@Override
